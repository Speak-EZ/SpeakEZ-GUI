 ...
 */
   ...
   function edit($id = null){
	    ...
	    // Prevent people from modifing homework help menu
	  if ($id == 15){

	     $this->Session->setFlash(__('To prevent people from breaking the homework help feature, this IVR menu has been locked. To change the digit corresponds to the homework help menu, please modify the "SpeakEZ" IVR menu. Contact me for more information. - L', true),'warning'); 
  	     $this->log("[WARNING] EDIT Homework help menu forbidden, Id: ".$id, "ivr");	
	     $this->redirect(array('action'=>'index')); 
	  }
	  ...
	  //Save submitted data.
		...
		foreach($this->request->data['IvrMenuFile'] as $key => $file){
          ...
          //Save text based form data
                	...
                	//print_r($this->request->data['Mapping']);
               //Edit existing mappings
                foreach($this->request->data['Mapping'] as $key => $entry){

                  //Add new mappings
                  //if((!$entry['type'] && ($entry['node_id']) || $entry['lam_id'] || $entry['ivr_id'])){
                  if((!array_key_exists('type', $entry) || $entry['type'] == '') && 
                  	($entry['node_id'] || $entry['lam_id'] || $entry['ivr_id'])){
   ...
   function record() {
		...
		$instance_id = $this->request->data['instance_id'];
		$message = $this->request->data['message'];
		$file = $_FILES['audioFile'];
		$file['fileName'] = $message . '.wav';

		//Upload one ore more wav files
		$fileOK = $this->uploadFiles($ivr_settings['path'].$instance_id."/".$ivr_settings['dir_menu'], array($file) ,false,'audio',true,true);
		...
		if(array_key_exists('urls', $fileOK)) {
			foreach ($fileOK['urls'] as $key => $url ){
				...
				var_dump($this->IvrMenu->saveField($field,$name));
				$this->Session->setFlash(__('The file %s was successfully uploaded.', $fileOK['original'][$key]), 'success', array(), $key);
				$this->log("[INFO] EDIT IVR, New file uploaded: ".$url, "ivr");
			}
		}
   ...
   function record2($id = null){

        $this->set('title_for_layout',  __('In-browser recording',true));           
