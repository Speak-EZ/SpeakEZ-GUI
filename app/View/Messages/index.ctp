 ...
 ***************************************************************************/
    ...
    $input2 = $this->Form->input('category', array('class' => 'ServiceType','type' => 'select', 'options' => $categories, 'label' => false, 'empty' => '-- '.__("Category",true).' --'));
      ...
      $input4 = $this->Form->input('service', array('class' => 'ServiceType','type' => 'select', 'options' => $lm_titles, 'label' => false, 'empty' => '--'.__("Message Type",true)."--"));
	...
	$sender     = $this->Access->showBlock($authGroup, $message['Message']['sender_name']!='' ? $message['Message']['sender_name'] : $message['Message']['sender']);
	...
	$listen     = $this->element('player',array('url'=>$message['Message']['url'],'file'=>$message['Message']['file'],'title'=>$title,'id'=>$message['Message']['id']));
     ...
     if($authGroup == 1) {
                   ...
                   $this->Form->submit(__('Download',true),  array('name' =>'data[Submit]', 'class' => 'button')),
