...
$this->Access->showButton($authGroup, 'IvrMenu', 'add', 'frameRightAlone', __('Create new',true), 'submit', 'button');
   ...
   echo $this->Session->flash(3);
	...
	echo "<div class ='instruction testnewclass'>".__("Audio files should be recorded in mono, 8KHz, and be maximum 10MB.",true)."</div>";
     	if ($ivr_menus){
	...
	foreach ($ivr_menus as $key => $ivr_menu){
		...
		$modified   = $this->Time->niceShort($ivr_menu['IvrMenu']['modified']);
          ...
          $record       = $this->Access->showBlock($authGroup, $this->Html->image("icons/listen.png", array("alt" => __("Record",true), "title" => __("Record",true), "url" => array("controller" => "ivr_menus", "action" => "record", $ivr_menu['IvrMenu']['id']))));
     		...
     		$row[$key] = array(
			   ...
			   $title,
			   $modified,
			   array($edit.' '.$record.' '.$delete,array('align'=>'center')));
			   }
